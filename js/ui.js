...
function renderCatalog(items, container) {
    ...
    const htmlString = items.map(item => {
        ...
        <div class="menu-item card" data-category="${item.category}" style="display: flex; flex-direction: column; padding: 15px;">
            ...
            <button class="btn-delete" data-id="${item.id}" style="background: #e74c3c; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; margin-top: 10px;">
    ❌ Видалити
</button>

<button class="btn-edit" data-id="${item.id}" style="background: #f39c12; color: white; border: none; padding: 5px 10px; border-radius: 4px; cursor: pointer; margin-top: 10px;">
    ✏️ Редагувати
</button>
        </div>
        `;
